...
Aurelia polyfills for browsers that don't support templates. However a few features of templates can't be polyfilled and requires workaround. In particular this is adding `<tamplate>` elements inside `<select>` and `<table>`. Following can't be done in a browser that nativly doesn't support templates
